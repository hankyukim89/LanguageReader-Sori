...
interface Props { article: Article; selectedLemma: string | null; selectLemma: (lemma: string | null) => void; isSaved: (lemma: string) => boolean; toggleSaved: (lemma: string) => void }

export function ReaderView({ article, selectedLemma, selectLemma, isSaved, toggleSaved }: Props) {
  ...
  useEffect(() => {
    if (!playing) return;
    const timer = window.setInterval(() => setProgress(current => current >= 96 ? 18 : current + 1), 900);
    return () => window.clearInterval(timer);
  }, [playing]);

  const togglePlayback = () => {
    if (playing) stopSpeaking(); else speakKorean(plainText);
  ...
  return <div className={`reader-layout ${selectedLemma ? 'word-open' : ''}`}><main className="reader">
    ...
    <div className="article-body">{article.paragraphs.map((paragraph,index) => <div className="article-paragraph" key={index}><p>{paragraph.segments.map((segment,segmentIndex) => segment.type === 'text' ? segment.value : <button key={segmentIndex} className="vocab-word" style={{'--word-color':LEVELS[article.level]} as React.CSSProperties} onClick={() => selectLemma(segment.lemma)}>{segment.value}</button>)}</p>{showTranslation ? <p className="english-translation" lang="en">{paragraph.english}</p> : null}</div>)}</div>
    <div className="reader-finish"><Sparkles/><div><strong>Nice reading!</strong><span>You completed a {article.level} story.</span></div><button>Mark complete</button></div>
