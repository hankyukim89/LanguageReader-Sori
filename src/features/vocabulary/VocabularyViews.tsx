...
interface SavedProps { saved: string[]; selectLemma: (lemma: string) => void }

export function SavedWordsView({ saved, selectLemma }: SavedProps) {
  return <div className="page saved-page"><header className="page-heading"><div><p>Your collection</p><h1>Saved words</h1><span>{saved.length} words ready to review.</span></div></header>
    {saved.length ? <div className="saved-list">{saved.map(lemma => { const entry = getVocabulary(lemma); if (!entry) return null; return <button key={lemma} onClick={() => selectLemma(lemma)} aria-label={`Open definition for ${entry.lemma}`}><i style={{background:LEVELS[entry.level]}}/><div><strong>{entry.lemma}</strong><span>{entry.pronunciation} · {entry.partOfSpeech}</span></div><p>{entry.meaning}</p><Bookmark fill="currentColor" aria-hidden="true"/></button>; })}</div> : <div className="empty-state"><Bookmark/><h2>No saved words yet</h2><p>Tap a highlighted open-class word while reading, then save it here.</p></div>}
  </div>;
}
...
export function ReviewView({ saved }: { saved: string[] }) {
  ...
  return <div className="page review-page"><header className="page-heading"><div><p>Daily practice</p><h1>Review</h1><span>A few minutes now goes a long way.</span></div></header><div className="review-count">Card {index+1} of {cards.length}</div>
    ...
    <div className="review-actions" aria-label="Review result">
      <button onClick={() => move(-1)} disabled={!flipped} title={!flipped ? 'Reveal the answer first' : undefined}><ChevronLeft/>Again</button>
      <button className="primary" onClick={() => move(1)} disabled={!flipped} title={!flipped ? 'Reveal the answer first' : undefined}>Got it<ChevronRight/></button>
    </div>
  </div>;
}
